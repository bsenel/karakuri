...
func TestBudgetedAgentChargesEveryCall(t *testing.T) {
	...
	agent := withBudget(inner, budget, nil, "twin-1", "obj-1")
...
func TestWithBudgetIsANoOpWithoutATwin(t *testing.T) {
	...
	inner := &greedyAgent{tokens: 10}
	if got := withBudget(inner, &recordingBudget{limit: 1}, nil, "", "obj-1"); got != coreagent.Agent(inner) {
		t.Error("an objective with no twin was wrapped")
	}
	if got := withBudget(inner, nil, nil, "twin-1", "obj-1"); got != coreagent.Agent(inner) {
